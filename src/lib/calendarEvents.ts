...
export function toCalendarEvents(
  ...
  for (const offering of offerings) {
    ...
    const location = provider.city ?? provider.region?.replace(/\n/g, ', ') ?? provider.country ?? ''
    const title = location
      ? `${course.official_name} — ${location} — ${provider.official_name}`
      : `${course.official_name} — ${provider.official_name}`
...
/** Sort events so same course types cluster together within each day. */
export function sortCalendarEvents(events: CalEvent[]): CalEvent[] {
  return [...events].sort((a, b) => {
    const dayA = a.start.toISOString().slice(0, 10)
    const dayB = b.start.toISOString().slice(0, 10)
    if (dayA !== dayB) return dayA.localeCompare(dayB)
    return a.resource.course.id.localeCompare(b.resource.course.id)
  })
}
