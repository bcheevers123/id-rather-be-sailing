# Task 8 Brief: Pipeline orchestrator and report builder

## Context

Task 8 of the "I'd Rather Be Sailing" plan. Tasks 1–7 are complete on branch `feature/maritime-training`. You are implementing the main pipeline orchestrator (`pipeline/generate.py`) and the coverage report builder (`pipeline/report.py`), plus one test.
...
from pipeline.pdf_parser import parse_pdf, ParsedPdf, RawProvider, RawApproval
...
def run_pipeline(dry_run: bool = False, output_dir: Path | None = None) -> None:
    """Main entry point. output_dir defaults to src/data/."""
```

## Step 1 — Write tests/pipeline/test_generate.py
...
**Troubleshooting the test:** The mock patches `pipeline.generate.download_mca_page`, `pipeline.generate.download_pdf`, and `pipeline.generate.fetch_pdf_links`. The test also patches `pipeline.generate.parse_pdf`. The `mock_download_pdf` returns `Path(tmp_path / "pst.pdf")` — note this file does NOT exist on disk, but `parse_pdf` is also mocked so it won't actually read the file. This is intentional.

If the test fails with a validation error, it's likely because the mock `RawProvider.contact_details` format doesn't produce valid contact parts — the field values in the test fixture are correct for `extract_contact_parts`.
...
- `run_pipeline` must never fabricate course data — it only writes data from the pipeline
- Schema validation via `validate_all` before writing JSON
- No scraping in the browser

## Report file

Write your report to: `.superpowers/sdd/2026-08-03-maritime-training-plan/task-8-report.md`

Return: Status, commit hash, test summary (N passed), concerns.
