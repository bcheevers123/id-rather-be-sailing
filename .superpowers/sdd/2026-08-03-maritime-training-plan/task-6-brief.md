# Task 6 Brief: Aliases, freshness, and change detection

## Context

Task 6 of the "I'd Rather Be Sailing" plan. Tasks 1–5 are complete on branch `feature/maritime-training`. You are implementing:
- `pipeline/aliases.json` — controlled alias + confusion-note table (used by frontend search)
- `pipeline/freshness.py` — computes freshness status from ISO timestamps
- `pipeline/change_detector.py` — detects significant data changes between pipeline runs
...
## Interfaces produced (used by Task 8 — pipeline orchestrator, and Task 10 — search)

```python
def compute_freshness(last_verified_iso: str | None, now_iso: str) -> str:
    """Returns: 'verified' (<24h), 'recently_checked' (1–7 days), 'stale' (>7 days or None)"""
...
def detect_changes(previous: dict, current: dict) -> list[Change]:
    """Returns list of Change objects describing significant differences."""
```
...
## Step 2 — Run failing tests (red phase)

```bash
pytest tests/pipeline/test_freshness.py tests/pipeline/test_change_detector.py -v
```
Expected: ImportError.

## Step 3 — Create pipeline/aliases.json
...
## Step 4 — Create pipeline/freshness.py
...
## Step 5 — Create pipeline/change_detector.py
...
pytest -v
```
All prior tests must pass too.

## Step 7 — Commit

```bash
git add pipeline/aliases.json pipeline/freshness.py pipeline/change_detector.py \
    tests/pipeline/test_freshness.py tests/pipeline/test_change_detector.py
git commit -m "feat: aliases table, freshness logic, change detection"
```

## Report file

Write your report to: `.superpowers/sdd/2026-08-03-maritime-training-plan/task-6-report.md`

Return: Status, commits, test summary (N passed), concerns.
